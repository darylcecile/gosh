...
| S18 | Egress to arbitrary origins/paths. | `parseAndValidateURL` accepts only `http`/`https`, exact normalized origin match, and optional path-prefix match unless `DangerouslyAllowFullInternet` is set (`commands/netcmd/netcmd.go`, `network.go`). | Enforced when network command is used. Dangerous full-internet flag bypasses origin allow-list by design. |
...
| S22 | SSRF to loopback/private/link-local/cloud metadata or DNS rebinding. | `parseAndValidateURL` rejects literal forbidden IPs when private-IP denial is active; custom `DialContext` resolves hostnames, rejects every forbidden resolved IP, and dials the checked IP, reducing DNS-rebinding risk; response size/timeouts are capped by `MaxResponseBytes`, `--max-time`, and context (`commands/netcmd/netcmd.go`). | Partly enforced. Important discrepancy: `shouldDenyPrivate` currently requires `NetworkPolicy.DenyPrivateIPs == true`; despite comments/PRD suggesting secure default, configuring `AllowedOrigins` without that flag does **not** deny private IPs. |
...
Mitigation: network commands fail when `NetworkPolicy.Enabled()` is false (`network.go`, `commands/netcmd/netcmd.go`). URL validation restricts scheme, method, exact origin, and optional path prefixes; redirects are manually handled and each hop is revalidated; max redirects and response bytes are capped (`commands/netcmd/netcmd.go`). When `DenyPrivateIPs` is true, literal private IPs are rejected and the custom dialer resolves all IPs, rejects private/loopback/link-local/unspecified/cloud-metadata addresses, then dials a checked IP rather than letting the transport re-resolve unchecked (`commands/netcmd/netcmd.go`).

Gap to note: private-IP denial is not automatic once network is enabled. Hosts must set `DenyPrivateIPs: true` unless they intentionally need private addresses (`network.go`, `commands/netcmd/netcmd.go`). This differs from the PRD's stated default-secure S22 posture.
...
Residual caveat: `MaxCallDepth`, `MaxMemoryBytes`, and some claimed expansion/glob/env bounds are not hard guarantees in the current code (`limits.go`; code search found no direct enforcement for `MaxGlobMatches`). Run `gosh` inside OS-level CPU/memory limits for hostile multi-tenant use.
...
5. **PRD/code discrepancies exist.** Notable gaps found while writing this model: `DenyPrivateIPs` is not automatic when network is enabled; `MaxGlobMatches` and env-byte caps are declared/claimed but no direct enforcement was found; AST validation does not visibly reject special redirect targets beyond the VFS; `goshfs` exposes a `NewReadWriteFS` pass-through despite the PRD saying true read-write FS is deferred (`network.go`, `commands/netcmd/netcmd.go`, `limits.go`, `engine.go`, `goshfs/goshfs.go`, `PRD.md`).
...
7. **Network policy is only as safe as host configuration.** `DangerouslyAllowFullInternet` disables the origin allow-list; broad allowed origins/methods/path prefixes or unset `DenyPrivateIPs` weaken SSRF defenses (`network.go`, `commands/netcmd/netcmd.go`).
